...
//			moveToCenter = true;
			...
			scrollViewportTo(left ? SWT.LEFT : SWT.RIGHT, _lastScrollbarPosition, diff);
		}
		else{
			scrollViewportTo(left ? SWT.LEFT : SWT.RIGHT, _lastScrollbarPosition, diff);
			//_gc.recalcSecBounds(true);
		}
		...
		// Force the update of the gantt chart
      _gc.forceFullUpdate();
		
      for (int j = 0; j < _gc.getEventListeners().size(); j++) {
	        ((IGanttEventListener) _gc.getEventListeners().get(j)).hScrollMoved();
		}
	...
	public void recalculate() {
      ...
      // Re-calculate the page increment (when clicking in the scrollbar, not in the arrows)
      // depending on the current view level
      int pageIncr = _scrollBar.getIncrement(); // Default value the same that the small increment
      switch(_gc.getCurrentView())
      {
//      case ISettings.VIEW_YEAR:
//         pageIncr = 1;
//         break;
      case ISettings.VIEW_MONTH:
         pageIncr *= 7; // 1 week step
         break;   
      case ISettings.VIEW_WEEK:
         pageIncr *= 3; // 3 days step
         break;   
      case ISettings.VIEW_DAY:
         pageIncr *= 6; // 6 hours step
         break;
      case ISettings.VIEW_HOURS:
         pageIncr *= 10; // 10 minutes step
         break;
      }
      
      _scrollBar.setPageIncrement(pageIncr);
		...
		if (_fixed) {
			...
			xLeftMostPixel = _gc.getXForDate(_gc.getInitStartDate());
			int xRightMostPixel = _gc.getRightMostPixel();
			
			if(_gc.getInitEndDate()!=null){
				xRightMostPixel = _gc.getXForDate(_gc.getInitEndDate());
			}
...
//			System.err.println("RANGE: " + Math.ceil(pixelRange) + " View: "+_gc.getCurrentView());// + " (debug1: " + debug1 + ", debug2: " + debug2 + "). Bonus: " + rangeBonus);
